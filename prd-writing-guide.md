...
## 🖼️ 전체 그림 (PRD 의 7 파트 스케치)

```text
┌─────────────────────────── PRD 전체 구조 (탑다운) ───────────────────────────┐
│                                                                              │
│  ┌─ Part 1. 두괄식 표지 ─────────────────────────────────────┐               │
│  │  🧭 한 줄 요약   🖼️ 전체 그림(TL;DR)   🎯 목표+비목표       │  ← 리뷰어    │
│  └─────────────────────────────────────────────────────────┘    2~3 분     │
│                                   │                                          │
│  ┌─ Part 2. 모듈 구성 ──────────────▼────────────────────────┐               │
│  │  🧱 모듈 표 (Services List)                                │  ← 리뷰어    │
│  │     └ orchestrator-worker-pattern-guide 분류로 매핑        │    5 분      │
│  └─────────────────────────────────────────────────────────┘               │
│                                   │                                          │
│  ┌─ Part 3. 주요 시나리오 (Depth 다단계) ────▼──────────────┐                │
│  │  🌲 Depth 1  Main 시나리오 (축 중심 내러티브)              │                │
│  │  🔍 Depth 2  모듈별 세부 (기동·정상·장애·중지)             │  ← 리뷰어    │
│  │  📌 Depth 3  관심 오브젝트 보강                            │    10 분     │
│  │     └ job-flow-diagram-guide 표기법 준수                   │                │
│  └─────────────────────────────────────────────────────────┘               │
│                                   │                                          │
│  ┌─ Part 4. 데이터 흐름 / I/O 경계 / 저장 ────▼────────────┐                 │
│  │  🔄 한 요청/메시지의 일생   ⚠️ 포화/에러 정책   📦 저장   │  ← 구현자    │
│  └─────────────────────────────────────────────────────────┘               │
│                                   │                                          │
│  ┌─ Part 5. 핵심 알고리즘 / 상태 (선택) ───▼────────────────┐                │
│  │  🎯 수식 · 임계값 · 상태 기계 · 규칙                       │  ← 구현자    │
│  │     └ state-diagram-guide 연결                             │                │
│  └─────────────────────────────────────────────────────────┘               │
│                                   │                                          │
│  ┌─ Part 6. 운영 / 관측성 ──────────▼────────────────────────┐               │
│  │  🩺 Health   📊 메트릭   📝 로그   🛡️ 에러·복구            │  ← 운영자    │
│  │  (로그는 `detailed-logging-prompt.md` 참조 의무)            │                │
│  └─────────────────────────────────────────────────────────┘               │
│                                   │                                          │
│  ┌─ Part 7. 설정 / 검증 / 확장 ─────▼────────────────────────┐               │
│  │  ⚙️ 설정값   🧪 테스트 전략   🗺️ 마이그레이션   🚀 향후    │  ← 구현자    │
│  └─────────────────────────────────────────────────────────┘               │
└──────────────────────────────────────────────────────────────────────────────┘

    읽는 방향 ↓     앞쪽만 읽어도 "전체를 알고", 뒤로 갈수록 "필요한 부분만" 파본다.
```

> 💡 **왜 7 파트인가?** Part 1~3 (요약·모듈·시나리오) 만으로 **리뷰어는 목적·구조·흐름을 파악** 할 수 있어야 한다. Part 4~7 은 **구현자와 운영자가 작업하며 참조** 하는 레퍼런스 영역이다. 리뷰 단계와 구현 단계의 독자를 분리하는 것이 두괄식의 핵심 목적이다.
...
**정의**: 시스템의 **입력 → 처리 → 출력** 관계를 한 장의 다이어그램으로 표현한다.

**형식 선택**:
- **ASCII 박스 다이어그램** (권장): 버전 관리에 친화적, diff 가능, 별도 렌더러 불요
- **Mermaid `graph LR` / `flowchart`** : 복잡도가 높아 ASCII 로 구조가 뭉개질 때

**필수 포함 요소**:
1. **외부 의존** (입력 소스): 예) HTTP 요청, 외부 API, 메시지 브로커, DB, 파일, cron 트리거, 사용자 입력
2. **주요 모듈 박스** (3~8 개): Part 2 의 모듈 목록의 subset
3. **주요 데이터 흐름** (화살표 3~6 개): `publish`, `query`, `emit`, `invoke` 등의 동사
4. **출력 대상**: 예) 응답, 저장소, 외부 API 호출, 메시지 발행, UI 렌더
5. 다이어그램 하단의 **"한 줄로 정리하면"** 으로 시작하는 1~2 문장 자연어 요약

**ASCII 다이어그램 범용 템플릿**:

```text
┌─────────────── <프로젝트명> (단일 프로세스 / 분산 / 서버리스) ────────────┐
│                                                                           │
<외부 입력>  ──▶  <진입 모듈>  ──┬──▶  <공유 채널/버스>  ──┐                │
                                 │                          │                │
                                 └──▶  <보조 경로 모듈>      │                │
                                                              │                │
                                                              ▼                │
...
**동기 요청-응답 시스템용 단순 템플릿**:

```text
<Client> ──▶ <Router> ──▶ <Handler> ──▶ <UseCase> ──▶ <Gateway> ──▶ <External>
                                                ▲
                                                │
                                         <Repository> ──▶ <DB>
```
...
- 리뷰어가 "이 시스템의 핵심 축이 뭐냐?" 답을 빨리 얻지 못한다.

**해결**: **Depth 1 = 축 중심 전체, Depth 2 = 모듈별 자세히, Depth 3 = 협력 오브젝트 보강** 의 3 층 구조.
...
| 축 | 역할 | 활성 시점 |
| --- | --- | --- |
| **<AxisA>** | <한 줄 책임> | **<상시 / 이벤트 기반 / 주기 / 단발>** |
| **<AxisB>** | ... | ... |
| **<AxisC>** | ... | ... |
```

**축 기반 jobflow 블록** (도메인 무관 템플릿):

```jobflow
master: Main(Orchestrator)
Object: Config, Logger, <AxisA>, <AxisB>, <AxisC>, <하위/협력 객체들...>

# ── 부팅 (의존 순서대로) ─────────────────────────────
Main.OnStart          --> Config.Load
Config.OnLoaded       --> Logger.Init
Logger.OnReady        --> <Gateway1>.Init
Main.OnStart          --> Health.Start
Main.OnStart          --> <AxisA>.Start
Main.OnStart          --> <AxisB>.Start
Main.OnStart          --> <AxisC>.Start

# ── <AxisA> 정상 흐름 ─────────────────────────────────
<External>.OnEvent    --> <AxisA>.Route
<AxisA>.OnProcessed   --> <SharedChannel>.Publish
<SharedChannel>.OnPublish --> <AxisB>.Feed

# ── <AxisB> 정상 흐름 (주기·단발 예) ──────────────────
<Trigger>.OnTick      --> <AxisB>.RunAction
<AxisB>.OnReady       --> <NextStep>

# ── <AxisC> 출력 흐름 ───────────────────────────────
<AnyModule>.OnSignal  --> <AxisC>.Emit
<AxisC>.OnEmit        --> <OutputGateway>.Send

# ── 셧다운 (상류 → 하류) ────────────────────────────
Main.OnSignal(SIGTERM) --> <AxisA>.Stop
<AxisA>.OnStopped     --> <SharedChannel>.Close
<SharedChannel>.OnClosed --> <AxisB>.Stop
<AxisB>.OnStopped     --> <AxisC>.Drain
<AxisC>.OnDrained     --> Health.Stop
```

**jobflow 표기 규칙** — [`job-flow-diagram-guide.md`](./job-flow-diagram-guide.md) 준수:
- `master:` 는 전체 조율자. 보통 `Main` 또는 최상위 `<Feature>Orchestrator`.
- `-->` 는 **하향식 호출** 또는 **이벤트 구독**. **수평 호출 금지** (Worker A → Worker B 직접 호출 없음).
- **메서드 파라미터는 쓰지 않는다**. 핵심 파라미터는 괄호로 힌트만: `(priority=HIGH)` 처럼.
- 구역 구분은 `# ── 섹션명 ──` 주석으로.
- 권장 섹션 순서: **부팅 → 축별 정상 흐름 → 셧다운**.
...
| `<AnyModule>.OnSignal` | <시그널 조건 만족 시> | `(priority, payload)` | `<AxisC>` |
| `Main.OnSignal(SIGTERM)` | OS 시그널 | — | 각 축 (역순) |
...
<외부 트리거 이벤트> 가 들어오면 `<AxisA>.OnEvent` 가 발사되고,
...
```jobflow
master: <ModuleName>
Object: <이 모듈의 하위/협력 객체들>

# ── 기동 ──────────────────────────────────────────
<ModuleName>.Start         --> <Dep1>.Init
<Dep1>.OnInit              --> <Dep2>.Connect
<Dep2>.OnConnected         --> <ModuleName>.state=RUNNING
...
<Filter>.OnDrop            --> MetricsRegistry.Inc(dropped_total)
...
**시나리오 1 — 기동**:
`<ModuleName>.Start` 호출 시 `<Dep1>` 을 먼저 초기화해 <크레덴셜/자원> 을
확보하고, 그 후 `<Dep2>.Connect` 로 <외부 연결> 을 수립한다. `<Dep2>` 가
`OnConnected` 를 발사하면 `<ModuleName>` 의 상태가 `RUNNING` 으로 바뀌고
정상 흐름에 진입한다.

**시나리오 2 — 정상 처리 한 건의 일생**:
<외부 입력 1 건> 이 `<Dep2>` 를 통해 수신되면 `<Parser>.Process` 가 호출되어
...
<외부 입력>
   │
   ▼
(1) <EntryModule>.receive()
       ├─ <분류·검증 결과로 라벨링>
       └─ 정규화 구조체: { field1, field2, ... }
   │
   ▼
(2) <FilterOrGate>.check()
       └─ 조건 불일치 시 drop/reject + 카운터 증가
   │
   ▼
(3) <Processor>.process()
       ├──▶ <Path A> ──▶ <OutputA>
       └──▶ <Path B> ──▶ <OutputB>
            (두 경로는 독립적, 한쪽 지연이 다른 쪽을 막지 않음)
```
````

**규칙**:
- 번호 `(1)`, `(2)`, `(3)` … 단계마다 **한 책임** 만.
- 분기는 트리 ASCII 로 시각화. `├──▶` / `└──▶` 사용.
- 드랍·폐기·필터링 경로는 주석으로 명시.

### 4-2. ⚠️ 포화 / 에러 정책 (필수 조건부)
...
> [`/Users/ryu/projects/project-guides/detailed-logging-prompt.md`](./detailed-logging-prompt.md)
...
### 6-4. ⏱️ 시간·타임존 규칙 (시간 기반 로직이 있을 때)
...
- [ ] 🌲 Depth 1 jobflow 가 job-flow-diagram-guide 규칙을 따른다 (수평 호출 없음, 파라미터 없음)
- [ ] 🔑 **Depth 1 jobflow 직후에 4 요소(①다이어그램 ②객체설명 ③이벤트설명 ④시나리오내러티브) 가 모두 있다**
- [ ] 🔑 **Depth 1 시나리오 내러티브가 2~3 개 (부팅·정상·셧다운 또는 장애) 있다**
- [ ] 🔍 Depth 2 섹션마다 기동·정상·장애·중지 4 경로가 모두 있다
...
실제 PRD 작성 시 관찰된 15 가지 실패 패턴. **하나라도 해당하면 리뷰 반려**.

| # | 안티 패턴 | 증상 | 대응 |
| --- | --- | --- | --- |
| 1 | **마케팅 카피 요약** | "혁신적인 …" | 구체 동사·구체 명사로 재작성 |
| 2 | **ASCII 다이어그램 실효 상실** | 구현 변경이 반영 안 됨 | PR 체크리스트에 "PRD 다이어그램 갱신 여부" 추가 |
...
| 6 | **Depth 2 = Depth 1 중복** | 장애 경로 없음 | Depth 2 는 장애 경로 필수 |
| 7 | **환경변수에 실제 값** | `API_KEY=abc123` | 이름과 기본값만, 값은 `—` |
| 8 | **외부 API 제약 누락** | rate limit·인증 만료 언급 없음 | 자가 복구 매트릭스에 명시 |
| 9 | **자연어 알고리즘** | "크게 증가하면" | 기호·수식·시간 창으로 재작성 |
| 10 | **테스트 전략 누락** | 검증 불가 | Part 7 에 4 층 전략 필수 |
| 11 | **역피라미드 붕괴** | 두괄식으로 시작 후 배경 설명 끼어듦 | 배경 설명은 해당 Part 내부 💡 박스로 |
| 12 | **jobflow 메서드 파라미터 표기** | `A.Save(user, id)` | 파라미터 제거, 힌트는 괄호로만 |
| 13 | **jobflow 만 두고 끝** | 객체 설명·이벤트 설명·내러티브 없음 | §3-1.5 **4 요소 세트** 의무 준수 |
| 14 | **시나리오 내러티브 없음** | 리뷰어가 "실제로 한 건이 들어오면 어떻게 흐르냐" 질문에 답 못 함 | 대표 시나리오 2~3 개를 문단으로 작성 |
| 15 | **이벤트 설명 표 없음** | `OnXxx` 가 jobflow 에만 나오고 의미 설명 없음 | 발사 조건·페이로드·수신자 3 열 표 의무 |
...
```text
                    ┌───────────────────────────┐
                    │  prd-writing-guide.md     │ ← 본 문서
                    └────────────┬──────────────┘
                                 │ 참조
         ┌───────────────────────┼────────────────────────────┐
         ▼                       ▼                            ▼
┌──────────────────┐  ┌────────────────────────┐  ┌─────────────────────────┐
│system-design-    │  │orchestrator-worker-    │  │job-flow-diagram-        │
│framework.md      │  │pattern-guide.md        │  │guide.md                 │
│(8 섹션 프레임워크)│  │(5 분류, 계층)          │  │(jobflow 표기법)          │
└──────────────────┘  └────────────────────────┘  └─────────────────────────┘
         │                       │                            │
         └───────────┬───────────┴────────────────────────────┘
                     ▼
         ┌────────────────────────────┐
         │detailed-logging-prompt.md  │  ← PRD Part 6 에서 참조 의무
         └────────────────────────────┘
         ┌────────────────────────────┐
         │state-diagram-guide.md      │  ← PRD Part 5 (FSM 있을 때)
         └────────────────────────────┘
         ┌────────────────────────────┐
         │navigation-diagram-guide.md │  ← PRD Part 3 (UI 있을 때)
         └────────────────────────────┘
         ┌────────────────────────────┐
         │screen-layout-guide.md      │  ← UI 프로젝트
         └────────────────────────────┘
         ┌────────────────────────────┐
         │code-structure-guidelines.md│  ← 구현 단계 참조
         └────────────────────────────┘
```
