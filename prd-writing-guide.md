...
> ⚠️ **jobflow `Object:` 라인에 하위 객체가 들어가 있으면 리뷰 반려** — Config / Logger / MetricsRegistry / Parser / Filter / WSConnection / Subscription / PriorityQueue / SpillStore 등은 Depth 2 에서 처음 선언되어야 한다. "Main 이 Logger 를 초기화한다" 같은 인프라 조립 절차는 Depth 1 의 관심사가 아니다.
...
**jobflow 표기 규칙** — [`job-flow-diagram-guide.md`](./job-flow-diagram-guide.md) 준수. 다음은 절대 금지:

| ❌ 금지 표현 | 이유 | ✅ 대체 |
| --- | --- | --- |
| `# 주석` / `# ── 섹션 ──` | jobflow 파서가 주석을 지원하지 않아 다이어그램이 깨진다 | 주석·구역 설명은 jobflow 블록 **밖 prose** 로 이동 |
| `A.Method(arg1, arg2)` | 파라미터 표기는 가이드에 없음 | 파라미터 생략 (`A.Method`), 힌트가 필요하면 블록 밖 설명에 |
| `A.Emit(priority=HIGH)` | 파라미터 힌트도 금지 | 우선순위별로 메서드를 분리: `A.EmitHigh`, `A.EmitUrgent` |
| `<Obj>.state=RUNNING` | 상태 대입은 가이드에 없음 | 상태 진입을 메서드로: `<Obj>.MarkRunning` |
| `forEach(x in list):` / 들여쓰기 | 반복 표현 금지 | 반복은 `A.Next --> B.Process` + `B.OnDone --> A.Next` 로 재귀 표현 |
| `sleep(N)` / `A.Pace(ms)` | 제어 함수 금지 | 페이싱이 중요하면 별도 `Pacer.Wait` 이벤트 객체로 |
| `emit(OnReady)` / `emit(...)` | 외부 함수 호출 금지 | 이벤트는 `A.OnReady --> B.Method` 로만 표현 |
| `A.OnFull(LOW)` | 열거값 파라미터 금지 | 값별로 이벤트 분리: `A.OnFullLow`, `A.OnFullHigh` |

**허용되는 표현** (전체 목록):

- `master: <Name>`, `Object: <Name1>, <Name2>, ...`
- `A.Method --> B.Method` (순차 호출)
- `A.OnEvent --> B.Method` (이벤트 구독)
- `A.Method.result --> B.HandleResult` (반환값)
- `A.Method.true --> B.HandleTrue` / `A.Method.false --> B.HandleFalse` (불리언 분기)
- `A.Method.Value1 --> B.HandleValue1` (enum 분기)

**권장 섹션 순서** (prose 에서 설명): 부팅 → 축별 정상 흐름 → 셧다운. jobflow 블록 내부에는 구역 경계를 표시하지 않는다 — 단순히 관련 라인을 **빈 줄** 로 구분해도 되고, 블록 밖의 prose 소제목·bullet 로 "이 부분은 부팅이다" 를 설명한다.
...
- [ ] 🧩 **jobflow 블록 안**에 `#` 주석·`forEach`·`sleep()`·`.state=`·파라미터(`(arg)`)·`master: Name(Type)` 같은 가이드 외 표기가 **하나도 없다**
- [ ] 🧩 **state / navigation 블록**도 전용 가이드에 정의된 문법만 사용 (가이드 외 표기 없음)
...
- [ ] 🌲 Depth 1 jobflow 가 job-flow-diagram-guide 규칙을 엄격히 따른다 (수평 호출 없음, 파라미터 없음, 주석 없음, 상태 대입 없음)
...
실제 PRD 작성 시 관찰된 19 가지 실패 패턴. **하나라도 해당하면 리뷰 반려**.
...
| 14 | **jobflow 메서드 파라미터 표기** | `A.Save(user, id)`, `A.Emit(priority=HIGH)` | 파라미터·힌트 전부 삭제. 값별로 메서드 분리(`A.EmitHigh`) |
| 15 | **jobflow 만 두고 끝** | 객체 설명·이벤트 설명·내러티브 없음 | §3-1.5 **4 요소 세트** 의무 준수 |
| 16 | **시나리오 내러티브 없음** | 리뷰어가 "실제로 한 건이 들어오면 어떻게 흐르냐" 질문에 답 못 함 | 대표 시나리오 2~3 개를 문단으로 작성 |
| 17 | **이벤트 설명 표 없음** | `OnXxx` 가 jobflow 에만 나오고 의미 설명 없음 | 발사 조건·페이로드·수신자 3 열 표 의무 |
| 18 | **jobflow 내부 주석 (`# ──`)** | jobflow 파서가 주석을 인식 못 해 다이어그램이 **깨진다** | 구역 설명은 블록 **밖 prose** 로, 섹션마다 jobflow 블록 분리 |
| 19 | **jobflow 내부 반복/제어 (`forEach`, `sleep()`)** | 가이드에 없는 문법으로 다이어그램이 깨진다 | 반복은 `A.Next → B.Process`/`B.OnDone → A.Next` 재귀 표현으로 |
| 20 | **jobflow 내부 상태 대입 (`.state=RUNNING`)** | `.state=` 는 가이드 구성 요소가 아니다 | 상태 진입을 메서드로: `.MarkRunning` |
| 21 | **jobflow 내부 외부 함수 호출 (`emit(...)`, `Init(size=N)`)** | 일반 언어 호출 표기 금지 | 이벤트는 `A.OnReady --> B.Method`, 파라미터는 prose 에 기술 |
| 22 | **jobflow `master:` 타입 주석** | `master: Main(Orchestrator)` | `master: Main` 만. 분류는 Part 2 모듈 표에서 확인 |
| 23 | **state / navigation 블록에 가이드 외 표기** | 괄호 속 설명, 파라미터, 주석 등 | 각 전용 가이드([state](./state-diagram-guide.md) / [navigation](./navigation-diagram-guide.md))의 문법만 사용 |
