...
> **독자는 신입 엔지니어다**: 모든 설명은 신입이 읽고 바로 이해할 수 있게 쉬운 말·짧은 문장으로 쓰고, 전문용어는 첫 등장에 한 번 풀어쓴다. 길고 장황한 설명과 **멋 부린 문장(비유·은유·과장·불필요한 수식어)은 금지한다** — 대상을 그대로 부른다.
...
서로 의존하지 않는 검색·페이지 방문·소스 확인은 무엇을 볼지 먼저 목록화한 뒤 한 응답에서 함께 요청한다 — 검색 하나에 턴 하나를 쓰지 않는다.
...
`clean` 이면 종료하고 **① 우선순위 로드맵 ② 채택하지 않은 패턴과 사유 ③ 접근 불가로 분석하지 못한 페이지 목록 ④ 후속 리서치가 필요한 미해결 질문 ⑤ 리서치 중 발견했으나 이번 범위에서 다루지 않은 무관한 문제(후속 과제)**를 보고한다.
