...
* 설계 설명·주석·문서는 신입 엔지니어가 읽는다고 생각하고 쉬운 말·짧은 문장으로 써줘. 전문용어는 첫 등장에 한 번 풀어쓰고, 길고 장황하게 쓰지 마.
* 로그 메시지·주석·보고문에 멋 부린 표현을 쓰지 마. 비유·은유·과장·불필요한 수식어를 빼고 무슨 일이 일어났는지만 그대로 써줘.
* 로깅 작업 중 발견한 무관한 버그는 이번 변경에서 고치지 말고 후속 과제로만 보고해줘.
...
따라서 `log_process_event()` 호출은 다음 규칙을 지킨다.

* **오케스트레이터의 코드 안에서 호출한다** — 워커/하위 객체 안에서 자기 자신을 위해 호출하지 않는다.
  워커는 자기 결과를 반환만 하고, 그 결과를 받아 "다음 단계로 보낸다" 는 결정과 그에 대한 이벤트
  기록은 오케스트레이터가 한다.
* 다이어그램의 `A --> B` 는 "오케스트레이터가 A 가 끝났으니 B 를 호출한다" 는 뜻이지, A 가 직접 B 를
  호출한다는 뜻이 아니다. 코드 구현도 이 모양을 따른다 — 오케스트레이터 메서드 안에서
  `r = a(); b(r)` 처럼 결과가 흐른다.
* 단계 사이마다 "오케스트레이터로 되돌아왔다 다시 나간다" 는 중간 이벤트를 굳이 만들지 않는다.
  화살표는 이미 오케스트레이터 관점이므로 그 round-trip 은 묵시적이다. 오케스트레이터가 실제로
  분기·가공·다른 메서드로의 책임 전환을 수행할 때만 그 지점을 별도 이벤트로 기록한다
  (예: `decision` phase, `synthesis` phase).
* 한 워커가 내부적으로 여러 LLM 호출을 한다면 그 호출들은 `log_llm_request()` 로 **아티팩트 로그**에
  남기고, 프로세스 이벤트로는 워커 단위 하나만 기록한다. 그렇지 않으면 다이어그램이 오케스트레이터
  관점이 아니라 워커 내부 관점으로 뒤섞인다.

이 원칙 덕분에 다이어그램을 읽는 사람은 "이 세션을 총괄한 객체가 무엇이고, 그 객체가 어떤 순서로 어떤
워커들에게 일을 위임했는지" 를 한 눈에 본다. 워커 내부 사정은 노드 클릭(아티팩트 링크)으로 들어간다.
...
- **`log_process_event()` 호출 위치는 오케스트레이터의 코드 안**으로 한정한다. 워커/하위 객체가 자기
  자신의 단계 진행을 직접 기록하지 않는다. 워커는 결과를 반환할 뿐이고, 그 결과를 받아 다음 단계로
  보내는 오케스트레이터의 메서드가 해당 단계의 `log_process_event()` 를 부른다 (잡플로우 다이어그램의
  화살표가 master 관점인 것과 동일한 원칙).
- 단, **아티팩트 로그**(`log_llm_request()`, `log_state_snapshot()`, 외부 호출 로그 등) 는 호출 주체가
  직접 남겨도 된다 — 이건 "내가 무엇을 했나" 의 사실 기록일 뿐, 플로우 결정이 아니기 때문.
  반환된 `artifact_path` 를 오케스트레이터에게 돌려주면, 오케스트레이터가 그 경로를 `log_process_event()`
  의 `artifact_path` 로 묶어 다이어그램 노드와 연결한다.
