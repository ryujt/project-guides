...
- **method-R 역추출**: 매크로(외부경계·진입점)→시스템(현 서비스/번들 경계)→모듈(서비스 내부 책임)→상세(복잡 워커). 핵심 문서 §5 에는 **대표 job flow 1장**(보통 시스템 계층)만 `jobflow` DSL 로(`orchestrator:`/`scope:` 헤더 명시) 두고, 나머지 계층·조각별 job flow 는 상세 파일에 둔다.
...
- **담는 것**: 계층·조각별 전체 `jobflow`, 시나리오별 `navigation`·`state`·`layout` 전체, 예외·오류 분기, 근거 표(관찰 항목·실제 경로/라인·현재 동작), 책임 소유 표, 데이터 모델·경계 계약 전문, 근거문서↔섹션↔이슈(R-0X) 매핑 표.
...
**P1 (권장)**: §0 두괄식 조감 누락 / 책임 소유 표 모호 / 3종 세트 누락(상세 흐름 링크는 `details/` 상세 파일로) / **핵심 문서 간명성 위반**(섹션당 대표 다이어그램 1개 초과·상세 수준 내용 혼입·화면 2~3장 초과) / **핵심 문서↔상세 파일 불일치**(§번호·`R-0X`·인덱스 누락·깨진 링크) / 6원칙 위반을 노출 대신 임의 교정.
