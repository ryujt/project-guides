...
> 원칙: **두괄식 · 다이어그램 1차 표현 · 간결·쉬운 설명 · project-guides 충실 준수**. 모든 설명은 **최대한 간결하고 쉬운 문장**으로 쓴다(짧은 문장·불필요한 수식어 제거·전문용어는 처음 1회 풀어쓰기). **독자는 신입 엔지니어다** — 처음 읽는 신입이 바로 이해할 수 있게 쓰고, 길고 장황한 설명은 금지한다. 종료는 정성 표현이 아니라 §설계-검증의 기계검증으로만 판정한다.
...
설계를 시작하기 전에 입력받은 요구사항을 확정한다. 요구사항이 모호하면 무엇이 불명확한지 선택지와 함께 적고, 답을 기다리며 멈추지 말고 가장 합리적인 해석을 가정 표에 명시한 뒤 설계를 끝까지 진행한다(확인이 필요한 질문은 종료 보고에 모아 남긴다).
...
- **현재 동작 요약**: 영향 조각의 현재 흐름을 3~6불릿으로. 전체 시스템 분석은 하지 않는다 — 전체 분석이 필요할 정도로 영향이 넓으면 **[AS-IS](./system-design-as-is-prompt.md)→[TO-BE](./system-design-to-be-prompt.md) 파이프라인으로 전환을 권고**하고 이 프롬프트의 범위에서 멈춘다 — 답을 기다리는 대기가 아니라 범위 판단이므로, 권고 사유·근거와 여기까지 확정한 내용을 보고로 남기고 종료한다.
...
- **멋 부린 문장** — 비유·은유·과장·불필요한 수식어를 쓴 설명
- 확인 질문·계획만 남기고 산출물을 완성하지 않음(자율 진행 위반)

**종료**: 위반 0 → 종료하고 **① 보류 FR 과 사유 ② Step 1 에서 발견한 무관 문제 목록(후속 과제 — 이번 설계에서 고치지 않음) ③ 구현 인계 사항 ④ 명시한 가정 중 사용자 확인이 필요한 항목** 을 보고한다. 이 설계서는 [PRD](../guides/prd-writing-guide.md) 의 해당 Part 입력, 또는 [멀티 에이전트 협업 프롬프트](./multi-agent-task-prompt.md)·[통합 테스트 프롬프트](./comprehensive-test-prompt.md)의 구현·검증 입력으로 인계할 수 있다.
