...
> 원칙: **두괄식 · 다이어그램 1차 표현 · 근거와 제안 분리 · 보편적 명명 · 간결하고 쉬운 설명**. 종료는 정성 판단이 아니라 아래 설계-검증 게이트로 판정한다.
...
flowchart TB
  ...
  DOC["Step 4 · 단일 설계 문서<br/>8섹션 + 계약/데이터/검증"]
...
1. **날짜·경로**: `DATE`를 `YYYY.MM.DD`로 한 번 확정한다. 산출물은 `docs/design/{DATE}/frontend-user/frontend-user-design.md` 단일 문서다.
...
`docs/design/{DATE}/frontend-user/frontend-user-design.md`를 아래 순서로 작성한다.

### §0. 한눈에 보기

- 요구사항 한 줄, 범위/범위 밖, 핵심 가정, 결정 3~5개
- 브라우저·프론트엔드·Auth/User API·IdP(선택)·DynamoDB·RDS(선택)의 신뢰 경계 조감도
- 사용자 상태별 대표 진입과 회원 수명주기를 요약한 다이어그램
...
먼저 전체 사용자 네비게이션 한 장을 만들고, 위 **공통 기준 네비게이션**을 실제 프로젝트 라우트·정책으로 치환해 아래 시나리오를 독립 `navigation` 블록으로 작성한다. 기준 흐름을 그대로 복사하지 말고 실제 Page/API 근거와 관련 FR을 붙인다.
...
- 구현 순서, feature flag/마이그레이션, 기존 세션·계정 이관, 롤백, 운영 준비 사항을 3~7개 불릿으로 인계한다.
...
작성 후 Critic 관점으로 검사한다. 위반이 있으면 지적된 섹션만 수정하고 다시 검사한다.
...
- §0 두괄식 조감 또는 다이어그램 하단 3~6개 설명이 없음
...
- `active_placeholder_count`: 최종 문서에 남은 `TODO`, `TBD`, `미정`, 빈 필수 셀 수
