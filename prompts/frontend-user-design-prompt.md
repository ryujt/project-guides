...
> 회원 데이터 저장소는 **특정 제품을 전제하지 않는다**. 프로젝트가 이미 쓰는(또는 선택한) 저장소를 입력으로 확정하고, 접근 패턴을 근거로 설계한다. 복잡한 조인·다중 엔터티 트랜잭션·강한 관계 무결성이 핵심인 비즈니스 데이터는 근거를 남기고 별도 저장소로 분리할 수 있다. 저장소 선택을 "인증 직접 구현"으로 해석하지 않는다.
...
> 원칙: **두괄식 · 다이어그램 1차 표현 · 근거와 제안 분리 · 보편적 명명 · 간결하고 쉬운 설명**. 산출물은 **간명한 핵심 문서 + 상세 파일 2계층**으로 작성한다 — 핵심 문서는 처음 읽는 사람이 5~10분 안에 전체 구조와 핵심 결정을 파악하는 문서고, 계약·데이터·검증의 깊이는 전부 상세 파일에 담는다. **독자는 신입 엔지니어다** — 모든 설명은 신입이 읽고 바로 이해할 수 있게 쉬운 말·짧은 문장으로 쓰고, 전문용어는 첫 등장에 한 번 풀어쓴다. 길고 장황한 설명과 **멋 부린 문장(비유·은유·과장·불필요한 수식어)은 금지한다** — 대상을 그대로 부른다. 종료는 정성 판단이 아니라 아래 설계-검증 게이트로 판정한다.
...
| 데이터 저장소 | 회원 데이터 저장소, 비즈니스 데이터 저장소, 데이터 소유권, 기존 연동 계약 |
...
- ASCII 박스·트리 다이어그램은 금지한다. 물리 데이터 설계를 논리 ERD 하나로 대신하지 않는다.
...
flowchart TB
  ...
  S3["Step 3 · 저장소 설계<br/>접근 패턴 · 저장소 경계"]
...
- 회원·프로필·식별자·동의와 애플리케이션이 소유하는 세션 메타데이터는 접근 패턴을 근거로 확정한 저장소에 설계한다.
...
5. **결정 로그**: 인증 방식, 로그인 식별자, 자동 로그인, 세션 저장 방식, 탈퇴 유예, 재가입 정책, 회원 데이터 저장소·모델 전략을 선택지·결정·근거로 기록한다.
...
- 사용자 스키마·테이블·인덱스·만료/삭제 처리·동시성 제어, 개인정보 보유 정책
...
- 인증 방식은 **관리형 IdP 사용** 또는 **전용 인증 서비스** 중 하나로 명확히 정한다. 관리형 IdP를 쓰면 위임한 자격 증명·연락처 검증·인증 세션의 Source of Truth(SoT)는 IdP이며 애플리케이션 저장소에 중복 저장하지 않는다. 애플리케이션 프로필은 `provider + subject`와 불변 `userId`의 매핑으로 연결한다.
...
- **관리형 IdP**는 위임 범위의 자격 증명, provider subject, token/session, 검증·인증 잠금 상태를 소유한다. 애플리케이션 저장소는 로컬 `userId`, provider-subject 매핑, 서비스 프로필·동의·서비스 계정 상태·탈퇴 작업을 기본으로 소유한다. `Session`은 필요한 경우 비권위 기기 표시 메타데이터로만 둔다.
- `provider + subject` 매핑은 "존재하지 않을 때만 쓰기" 조건부 생성으로 덮어쓰기를 막는다. 로컬 profile·provider map·가입 intent finalize는 가능한 한 하나의 원자적 쓰기(트랜잭션)로 묶는다. 이미 매핑이 있으면 새 사용자를 만들지 않고 기존 `userId`로 수렴하며, IdP 성공 뒤 finalize 충돌은 기존 매핑 재조회·멱등 재시도·reconciliation로 복구한다.
- **자체 인증**은 분리한 인증 저장소(또는 전용 접근 경계를 가진 애플리케이션 저장소)에 `IdentityClaim`, `Session`, `VerificationChallenge`, `PasswordResetChallenge`를 설계하고 전용 인증 서비스만 접근하게 한다.
- 애플리케이션 저장소의 트랜잭션은 IdP 변경까지 원자화하지 못한다. 관리형 IdP 가입·식별자 변경·탈퇴는 `멱등 intent/예약 → IdP 변경 → 로컬 finalize` 또는 근거 있는 역순으로 설계하고, 단계 상태·실패 정리·재시도·정합성 점검을 둔다. 이를 단일 트랜잭션이라고 표현하지 않는다.
- `IdP 상태 ↔ 애플리케이션 계정 상태 ↔ 세션 수락 여부` 전이 표를 만들고, 유예 탈퇴 중 IdP disable/delete와 취소 시 재활성화 순서를 정한다.
...
엔터티/ERD부터 그리지 말고 접근 패턴을 먼저 작성한다. 각 패턴에 `호출 주체 · 빈도 · 키/조회 조건 · 정합성 · 물리 키/인덱스 · 원자성(고유 제약·조건부 쓰기·트랜잭션) · 만료 처리 · 멱등성`을 매핑한다.

최소 접근 패턴은 다음과 같다. IdP에 위임한 패턴은 애플리케이션 저장소 모델을 만들지 말고 `IdP 계약 · 앱에서 필요한 결과 · 실패 처리`로 대체한다.
...
- 키 설계는 저장소 특성에 맞게 항목별로 확정한다. 기본 조회 키 후보는 프로필의 불변 `userId`, provider 매핑의 `provider + subject`, 자체 인증 식별자의 정규화 지문, 세션의 세션 지문, 멱등 요청의 `principal + operation + key`다. 하나의 키 규칙을 모든 항목에 강제하지 않는다.
- 원문 이메일·휴대폰을 키에 노출하지 않는다. 식별자별 정규화 정책을 명시하고 이메일을 무조건 전체 소문자화하지 않는다. 휴대폰은 적용 국가 정책에 맞는 E.164 등 표준 형식을 검토한다. 지문(해시/HMAC)을 쓰면 key version·충돌 처리·키 관리 접근 권한을 설계하고, 회전 중 모든 활성 버전 조회와 dual-write/rekey, 이전 버전 폐기 조건으로 고유성을 유지한다.
- 식별자 고유성은 비원자적 선조회(조회 후 쓰기)나 최종 일관성 인덱스 조회로 보장하지 않는다. 고유 제약, 조건부 쓰기, 트랜잭션 등 저장소가 제공하는 원자적 수단으로 프로필·식별자 소유권을 원자 생성한다.
- 미검증 식별자 예약은 만료 시각과 상태를 조건부 검사해 자동 삭제를 기다리지 않고 재선점할 수 있게 한다. 식별자 변경은 새 claim 예약→검증→프로필 교체→기존 claim 해제의 트랜잭션 또는 saga로 설계한다.
- 자체 인증의 저엔트로피 OTP는 서버 비밀키 기반 HMAC verifier, 짧은 만료, 원자적 시도 차감·일회성 소비, 발송/검증 rate limit을 함께 적용한다. 고엔트로피 reset/verification token과 refresh token은 원문이 아닌 verifier/지문만 저장한다. 모든 만료는 만료 시각을 직접 검사하고, 저장소의 자동 만료 삭제(TTL)는 지연 정리·비용 절감 용도로만 사용한다.
...
- 프로필 변경은 `version` 조건 검사(낙관적 잠금)로 동시 수정에 의한 덮어쓰기를 막는다. API에는 충돌(409) 복구 흐름을 둔다.
- 보조 인덱스는 실제 조회 패턴별로 최소한만 둔다. 요청 경로의 전체 스캔, 무한히 자라는 데이터 집합, 특정 키로의 부하 집중(hot key), 저장소별 항목 크기 한도 위험을 검토한다.
- 접근 패턴별 예상 요청량·데이터 크기·읽기 일관성·인덱스 쓰기 증폭·백업 비용을 추정하고, 저장소의 과금·용량 모델에 맞는 운영 방식을 근거와 함께 선택한다.
- 백업/시점 복구, 저장 시 암호화, 최소 권한 접근 제어, 데이터 분류별 보유·삭제 정책을 포함한다. 복원 후 탈퇴 데이터가 되살아나지 않도록 최소 삭제 ledger/tombstone을 재적용하는 절차를 두고 legal hold와 일반 보존을 구분한다. 고용량 감사 이벤트는 사용자 데이터 집합에 계속 누적하지 않는다.
- 테이블/컬렉션 구성은 접근 패턴·격리·비용 근거로 선택한다. 저장소의 관행을 근거 없는 전제로 두지 않는다.
- 다중 리전 쓰기를 선택하면 복제만으로 전역 고유성을 가정하지 않는다. 식별자·provider 매핑의 단일 writer/home region 또는 별도 전역 고유성 조정자를 정하고 장애 전환 중 쓰기 정책을 명시한다.

### 저장소 분리 기준

회원 저장소와 성격이 다른 저장소를 분리해서 쓰는 기준이다. 아래 조건이 실제로 필요한 영역만 분리 후보로 둔다.

- 여러 엔터티에 걸친 강한 관계형 트랜잭션과 참조 무결성
- 복잡한 조인·집계·가변 조건 보고서가 핵심인 조회
- 주문·정산·계약처럼 관계형 제약 자체가 비즈니스 규칙인 데이터

회원 프로필·동의·서비스 계정 상태의 기본 SoT는 회원 저장소에 유지한다. 복잡한 회원 리포트는 읽기 모델/분석 저장소를 먼저 검토하고, 관계형 비즈니스 규칙이 소유하는 데이터만 분리 저장소의 SoT로 둔다.

저장소를 분리하면 `데이터/필드 · SoT · 쓰기 주체 · 읽기 주체 · 동기화 지연 허용치 · 삭제 책임` 소유권 표를 작성한다. 한 요청에서 프론트엔드 또는 API가 두 저장소를 무보호 이중 쓰기하지 않는다. 변경 이벤트 스트림 또는 transactional outbox의 소유 저장소, event version·ordering, retry/DLQ, 멱등 inbox, reconciliation 주기와 허용 지연을 정하고 두 저장소는 불변 `userId`로 연결한다. 부가 저장소 장애가 기본 로그인까지 불필요하게 전파되지 않도록 의존 방향을 정한다.

### 탈퇴 오케스트레이션

- 저장소 구성과 무관하게 IdP·회원 저장소·객체 저장소·메시징·외부 비즈니스 저장소 중 둘 이상을 건드리면 분산 삭제 작업으로 설계한다.
...
1. **한눈에 보기**: 요구사항 한 줄, 범위/범위 밖, 핵심 가정 + 브라우저·프론트엔드·Auth/User API·IdP(선택)·회원 저장소·분리 저장소(선택)의 신뢰 경계 조감도 1장.
2. **핵심 결정 표**: 인증 방식, 로그인 식별자, 세션 저장 방식, 탈퇴 유예/재가입, 회원 데이터 저장소·모델 전략 등 Step 0 결정 로그의 확정본을 `결정 · 선택지 · 근거 한 줄`로.
...
6. **데이터 한눈에**: 논리 `erDiagram` 1장 + `항목 · SoT · 저장소` 표. 접근 패턴·물리 키/인덱스·저장소 경계 상세는 상세 파일로.
...
- 공통 앱 데이터와 인증 방식별 데이터를 분리한다. 관리형 IdP에 위임한 자격 증명·token/session·challenge는 IdP 계약으로, 앱 프로필·동의·서비스 계정 상태·탈퇴 작업은 애플리케이션 저장소 데이터로 작성한다.
...
- Auth API, User API, IdP, user repository, deletion worker, 알림 gateway의 경계
...
- 논리 관계 `erDiagram`, 접근 패턴 표, 물리 모델(키/인덱스) 표를 모두 작성한다.
- 인증 방식별 SoT 표와 관리형 IdP 연계 intent/finalize/재처리 상태를 작성한다. IdP 위임 항목은 애플리케이션 저장소 모델을 만들지 않는다.
- 각 데이터 항목은 키, 주요 속성, 무결성·원자성 조건, 만료 처리, 민감도, 생성/갱신/삭제 주체, 관련 FR을 가진다.
- 고유성·동시성·멱등성·만료 지연·부하 집중·비용 최적화 근거를 별도 소절로 설명한다.
- 분리 저장소가 없으면 "불필요" 근거를 적고, 있으면 SoT/동기화/실패 복구/탈퇴 처리 표를 작성한다.
...
- 단위, 컴포넌트, API 계약, 통합, E2E, 접근성, 보안, 동시성, 만료 지연, 장애 복구 테스트를 설계한다.
...
- 관리형 IdP와 애플리케이션 저장소의 SoT, subject↔userId 매핑, 비원자 연계 단계·실패 재처리가 없음
- 관리형 IdP 선택 시 provider-subject 조건부 고유 매핑, profile/map/intent finalize 원자성, 기존 매핑 수렴, finalize 충돌 복구 중 하나가 없음
- 저장소 접근 패턴과 물리 키/인덱스 설계가 매핑되지 않음
- 고유성을 비원자적 선조회만으로 보장하거나 지연 자동 삭제(TTL)를 만료 판정의 즉시성 근거로 사용함
...
- 저장소 분리 근거·SoT·동기화·이중 쓰기 방지 또는 탈퇴 saga가 불명확함
- 접근성, 민감 로그 차단, 감사 이벤트, 경합/TTL 지연 테스트가 없음
- 결정하지 않은 정책을 가정 표에 남기지 않고 암묵적으로 선택함
- 멋 부린 문장(비유·은유·과장·불필요한 수식어)을 사용함
- 확인 질문·계획만 남기고 문서 세트를 완성하지 않음(자율 진행 위반)
...
`clean`이면 종료하고 **① 보류한 Should/Optional FR과 사유 ② 저장소 선택과 분리 영역(또는 분리 불필요) 근거 ③ 구현 인계 사항 ④ 참고 구현에서 의도적으로 채택하지 않은 항목 ⑤ 분석 중 발견했으나 이번 범위에서 다루지 않은 무관한 문제(후속 과제) ⑥ 사용자 확인이 필요한 가정**을 보고한다.
