...
> 원칙: **두괄식 · 다이어그램 1차 표현 · 근거와 제안 분리 · 보편적 명명 · 간결하고 쉬운 설명**. 산출물은 **간명한 핵심 문서 + 상세 파일 2계층**으로 작성한다 — 핵심 문서는 처음 읽는 사람이 5~10분 안에 전체 구조와 핵심 결정을 파악하는 문서고, 계약·데이터·검증의 깊이는 전부 상세 파일에 담는다. **독자는 신입 엔지니어다** — 모든 설명은 신입이 읽고 바로 이해할 수 있게 쉬운 말·짧은 문장으로 쓰고, 전문용어는 첫 등장에 한 번 풀어쓴다. 길고 장황한 설명은 금지한다. 종료는 정성 판단이 아니라 아래 설계-검증 게이트로 판정한다.
...
`clean`이면 종료하고 **① 보류한 Should/Optional FR과 사유 ② 저장소 선택과 분리 영역(또는 분리 불필요) 근거 ③ 구현 인계 사항 ④ 참고 구현에서 의도적으로 채택하지 않은 항목**을 보고한다.
